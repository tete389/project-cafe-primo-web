...
export default function DialogConfirmPayment(params) {
  const { openConfirmPayment, setOpenConfirmPayment, url } = params;
  // const shopSettingInfo = useContext(ShopSettingContext);

  // const [phoneCollect, setPhoneCollect] = useState({
  //   phoneNumber: "",
  //   collectPoint: shopSettingInfo
  //     ? shopSettingInfo?.pointCollectRate * openConfirmOrder.orderPrice
  //     : 0,
  //   collectMore: "",
  // });
  ...
  //           Number(openConfirmOrder.orderPrice) +
      ...
      <div className="modal-box w-max">
        ...
        <div className="mt-0 modal-action">
          {resUpdateStatusState.isLoadingUpdate ? (
            ...
            <button
              className="btn btn-active"
              onClick={() => sendUpdateStatusOrder(openConfirmPayment.orderId)}
            >
              ยืนยัน
            </button>
          )}
